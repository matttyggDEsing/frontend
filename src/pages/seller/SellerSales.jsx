...
function printSale(sale) {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Venta #${sale.id}</title>
  <style>
    ...
    <h1>Venta #${sale.id}</h1>
    <p class="muted">${new Date(sale.created_at).toLocaleString('es-AR')}</p>
    <div class="row"><span>Cliente</span><strong>${sale.first_name} ${sale.last_name}</strong></div>
    <div class="row"><span>Método de pago</span><strong>${sale.payment_method}</strong></div>
    <div class="row"><span>Estado</span><strong>${STATUS_STYLE[sale.status]?.label || sale.status}</strong></div>
    ...
    ${sale.notes ? `<p class="muted" style="margin-top:16px">Obs: ${sale.notes}</p>` : ''}
  </body></html>`
